...
(function () {
  ...
  var today = new Date();
  var state = { view: (prefs.defaultView || 'week'), current: today, tasks: loadTasks() };
  ...
  function pastelFor(n) {
    ...
    var tones = [
      { fg: '#64748b', ring: '#cbd5e1' }, // 0
      { fg: '#0e7490', ring: '#a5f3fc' }, // 1 cyan
      { fg: '#7c3aed', ring: '#ddd6fe' }, // 2 violet
      { fg: '#a16207', ring: '#fde68a' }, // 3 amber
      { fg: '#16a34a', ring: '#bbf7d0' }, // 4 green
      { fg: '#0ea5e9', ring: '#bae6fd' }, // 5 blue
      { fg: '#db2777', ring: '#fbcfe8' }  // 6 rose
    ];
    return tones[bucket];
  }
  ...
  function renderWeek() {
    ...
    for (var i = 0; i < 7; i++) {
      ...
      head.innerHTML =
        ...
        '<button class="p-day__count" style="color:' + tone.fg + ';border-color:' + tone.fg + ';" data-open="' + ymd + '">' + count + '</button>';
  ...
  if (addEventBtn) {
    ...
    addEventBtn.addEventListener('click', function (e) { e.preventDefault(); openSheet(); });
  }
  ...
  function wireExitButton() {
    ...
    btnExit.addEventListener('click', function (e) { e.preventDefault(); handleLogout(); });
  }
  wireExitButton();
  ...
  function clearAuthAll() {
    try {
      ['authUser', 'authName', 'token', 'auth.token', 'auth.user', 'looz:justLoggedIn', 'looz:loggedOut']
        .forEach(function (k) { try { localStorage.removeItem(k); } catch (e) { } try { sessionStorage.removeItem(k); } catch (e) { } });
    } catch (e) { }
  }
  ...
  (function intro() {
    ...
    if (!document.getElementById('intro-style')) {
      var s2 = document.createElement('style'); s2.id = 'intro-style';
      s2.textContent = `
