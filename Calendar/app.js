...
(function () {
      ...
      (function (node) {
        ...
        function step(ts) {
          ...
          node.style.transform =
            'translate(' + (tx*ease) + 'px,' + (ty*ease) + 'px) rotate(' + (rot*ease) + 'deg) scale(' + (1 - 0.2*ease) + ')';
  ...
  if (lemonToggle && appNav) {
    lemonToggle.addEventListener('click', function () {
      if (hasClass(appNav, 'u-is-collapsed')) remClass(appNav, 'u-is-collapsed');
      else addClass(appNav, 'u-is-collapsed');
    });
  }
  ...
  function openSheet() {
    ...
    try { titleInput && titleInput.focus(); } catch (e) {}
  }
  function closeSheet() {
    if (!sheet) return;
    remClass(sheet, 'is-open');
    setTimeout(function(){ addClass(sheet, 'u-hidden'); }, 220);
  }
  ...
  if (taskList) {
    ...
    taskList.addEventListener('change', function (e) {
      ...
      li.addEventListener('animationend', onAnimEnd, { once: true });
    });
  }
  ...
  (function () {
    const btn = document.getElementById('lemonToggle');
