...
(function () {
    ...
    for (var i = 0; i < COUNT; i++) {
      ...
      (function (node) {
        ...
        function step(ts) {
          ...
          else node.parentNode && node.parentNode.removeChild(node);
        }
  ...
  function openSheet() {
    ...
    if (titleInput) try { titleInput.focus(); } catch (e) {}
  }
  function closeSheet() {
    if (!sheet) return;
  ...
  if (sheetPanel) {
    ...
    sheetPanel.addEventListener('click', function (e) { e.stopPropagation(); });
  }
  ...
  if (taskList) {
    ...
    taskList.addEventListener('change', function (e) {
      ...
      if (li) li.addEventListener('animationend', onAnimEnd, { once: true });
    });
  }
