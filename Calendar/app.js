...
(function () {
  ...
  function remClass(el, c) { if (el && el.classList) el.classList.remove(c); }
  ...
  (function initNav() {
    ...
    function openNav() {
      ...
      nav.addEventListener('transitionend', function onEnd(e) {
        if (e.propertyName === 'max-height') { nav.style.maxHeight = 'none'; nav.removeEventListener('transitionend', onEnd); }
      });
      btn.setAttribute('aria-expanded', 'true');
    }
    function closeNav() {
      var current = nav.scrollHeight; nav.style.maxHeight = current + 'px'; nav.offsetHeight;
      nav.style.maxHeight = '0'; btn.setAttribute('aria-expanded', 'false');
      nav.addEventListener('transitionend', function onEnd(e) {
        if (e.propertyName === 'max-height') { nav.classList.add('u-is-collapsed'); nav.removeEventListener('transitionend', onEnd); }
      });
    }
    btn.addEventListener('click', function () { hasClass(nav,'u-is-collapsed') ? openNav() : closeNav(); });
  })();

  /* ========= Storage ========= */
  var STORAGE_KEY = 'plannerTasks';
  var state = {
    view: 'day',                   // default DAILY (first thing shown)
    current: new Date(),           // anchor date
    tasks: loadTasks()
  };
  ...
  /* ----- DAY ----- */
  function renderDay(){
    var root = plannerRoot; root.innerHTML='';
    var wrap = document.createElement('div'); wrap.className='p-dayview';

    var h = document.createElement('div'); h.className='p-dayview__head';
    ...
    var items = state.tasks.filter(t=>t.date===ymd).sort((a,b)=>a.time.localeCompare(b.time));

    if (!items.length){
      var empty = document.createElement('div'); empty.className='p-empty'; empty.textContent='אין אירועים ליום זה.';
  ...
  function renderWeek(){
    var root = plannerRoot; root.innerHTML='';
    var wrap = document.createElement('div'); wrap.className='p-week';
    ...
    for (var i=0;i<7;i++){
      ...
      var box = document.createElement('div'); box.className='p-day';
      var head = document.createElement('div'); head.className='p-day__head';
      head.innerHTML = '<span class="p-day__name">'+HEB_DAYS[i]+'</span> '+
                       '<span class="p-day__date" data-goto="'+ymd+'">'+pad2(day.getDate())+'.'+pad2(day.getMonth()+1)+'</span>';
      box.appendChild(head);

      var dayTasks = state.tasks.filter(t=>t.date===ymd).sort((a,b)=>a.time.localeCompare(b.time));
  ...
  function renderMonth(){
    var root = plannerRoot; root.innerHTML='';
    var grid = document.createElement('div'); grid.className='p-month';
    ...
    for (var i=0;i<42;i++){
      ...
      var head = document.createElement('div'); head.className='p-cell__date';
      head.textContent = day.getDate(); head.dataset.goto = ymd;
      cell.appendChild(head);

      var list = document.createElement('div'); list.className='p-cell__tasks';
      var items = state.tasks.filter(t=>t.date===ymd).sort((a,b)=>a.time.localeCompare(b.time));

      // show up to 2 chips
      items.slice(0,2).forEach(function(t){
        var chip = document.createElement('div'); chip.className='p-chip'; chip.textContent = t.time+' · '+t.title;
        list.appendChild(chip);
      });
      if (items.length>2){
        var more = document.createElement('div'); more.className='p-chip'; more.textContent='…';
        list.appendChild(more);
      }
      cell.appendChild(list);

      if (ymd === curKey){
        var cnt = document.createElement('span'); cnt.className='p-count'; cnt.textContent = String(items.length);
  ...
  plannerRoot && plannerRoot.addEventListener('click', function(e){
    ...
    // go to a specific day from week/month
  ...
  function openSheet() { if (!sheet) return; remClass(sheet,'u-hidden'); addClass(sheet,'is-open'); try{ titleInput&&titleInput.focus(); }catch(e){} }
  function closeSheet(){ if (!sheet) return; remClass(sheet,'is-open'); setTimeout(function(){ addClass(sheet,'u-hidden'); },220); }
  ...
  function confettiAtEl(el) {
    ...
    var COUNT=52, MIN_DIST=60, MAX_DIST=160, DURATION=1200;
    for (var i=0;i<COUNT;i++){
      var p=document.createElement('span'); p.className='c-confetti';
      ...
      (function(node){
        ...
        var tx=Math.cos(angle)*dist, ty=Math.sin(angle)*dist*(0.75+Math.random()*0.5);
        var rot=(Math.random()*720-360), start=null;
        function step(ts){ if(!start) start=ts; var t=Math.min(1,(ts-start)/DURATION), ease=t*(2-t);
  ...
  /* ========= Logout ========= */
  function logout() {
    try { localStorage.removeItem('authUser'); localStorage.removeItem('token'); sessionStorage.clear(); } catch (e) {}
